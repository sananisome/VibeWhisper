...
2026.4.9:
* 修复批量上传时首个文件不导出 LRC/SRT 的 bug。

2026.4.8:
* 项目从 N46Whisper 重命名为 VibeWhisper。
* `srt2lrc` 拆分为独立模块。
* 修复多个 bug：SRT 末尾数字行越界、LRC 多行字幕丢失、未知字幕样式崩溃、翻译 cell 导入错误。
...
## 如何使用
* [点击这里](https://colab.research.google.com/github/sananisome/VibeWhisper/blob/main/VibeWhisper.ipynb) 在Google Colab中打开应用。
* 上传要识别的文件并运行应用
* 识别完成后ass文件会自动下载到本地.
...
2026.4.8:
* 项目重命名为 VibeWhisper，拆分 srt2lrc 独立模块，修复多个 bug。
